...
GDP_destination <- GDP %>%
  ...
  select(-GDP_agri_quintile, -GDP_agri_quartile, -GDP_agri_tertile)
...
g <- fepois(migrates ~ SPEI:GDP_agri_quintile + sw0(SPEI_droughts_intense:GDP_agri_quintile + SPEI_floods_intense:GDP_agri_quintile) |  origin + destination^year + origin^destination, fixef.rm = "none", migration)
g2 <- fepois(migrates ~ SPEI:GDP_agri_quartile + sw0(SPEI_droughts_intense:GDP_agri_quartile + SPEI_floods_intense:GDP_agri_quartile) |  origin + destination^year + origin^destination, fixef.rm = "none", migration)
g3 <- fepois(migrates ~ SPEI:GDP_agri_tertile + sw0(SPEI_droughts_intense:GDP_agri_tertile + SPEI_floods_intense:GDP_agri_tertile) |  origin + destination^year + origin^destination, fixef.rm = "none", migration)


etable(g, g2, g3, cluster = "origin")


#Tests - No rich countries
x <- migration %>%
  filter(GDP_agri_quintile != 1) %>%
  mutate(agri = ifelse(GDP_agri_quintile == 5, 1, 0))

y <- migration %>%
  filter(GDP_agri_quartile != 1) %>%
  mutate(agri = ifelse(GDP_agri_quartile == 4, 1, 0))

z <- migration %>%
  filter(GDP_agri_tertile != 1) %>%
  mutate(agri = ifelse(GDP_agri_tertile == 3, 1, 0))

g <- fepois(migrates ~ SPEI + SPEI_droughts_intense + SPEI_floods_intense + sw0(SPEI:agri + SPEI_droughts_intense:agri + SPEI_floods_intense:agri) |  origin + destination^year + origin^destination, fixef.rm = "none", x)
g2 <- fepois(migrates ~ SPEI + SPEI_droughts_intense + SPEI_floods_intense + sw0(SPEI:agri + SPEI_droughts_intense:agri + SPEI_floods_intense:agri) |  origin + destination^year + origin^destination, fixef.rm = "none", y)
g3 <- fepois(migrates ~ SPEI + SPEI_droughts_intense + SPEI_floods_intense + sw0(SPEI:agri + SPEI_droughts_intense:agri + SPEI_floods_intense:agri) |  origin + destination^year + origin^destination, fixef.rm = "none", z)

etable(g, g2, g3, cluster = "origin")
etable(g, g2, g3, cluster = ~origin + destination)

#Tests - No poor countries
x <- migration %>%
  filter(GDP_agri_quintile != 5) %>%
  mutate(rich = ifelse(GDP_agri_quintile == 1, 1, 0))

y <- migration %>%
  filter(GDP_agri_quartile != 4) %>%
  mutate(rich = ifelse(GDP_agri_quartile == 1, 1, 0))

z <- migration %>%
  filter(GDP_agri_tertile != 3) %>%
  mutate(rich = ifelse(GDP_agri_tertile == 1, 1, 0))

g <- fepois(migrates ~ SPEI + SPEI_droughts_intense + SPEI_floods_intense + sw0(SPEI:rich + SPEI_droughts_intense:rich + SPEI_floods_intense:rich) |  origin + destination^year + origin^destination, fixef.rm = "none", x)
g2 <- fepois(migrates ~ SPEI + SPEI_droughts_intense + SPEI_floods_intense + sw0(SPEI:rich + SPEI_droughts_intense:rich + SPEI_floods_intense:rich) |  origin + destination^year + origin^destination, fixef.rm = "none", y)
g3 <- fepois(migrates ~ SPEI + SPEI_droughts_intense + SPEI_floods_intense + sw0(SPEI:rich + SPEI_droughts_intense:rich + SPEI_floods_intense:rich) |  origin + destination^year + origin^destination, fixef.rm = "none", z)
...
x <- migration %>%
  filter(GDP_agri_destination_quintile != 5) %>%
  mutate(rich = ifelse(GDP_agri_destination_quintile == 1, 1, 0))

y <- migration %>%
  filter(GDP_agri_destination_quartile != 4) %>%
  mutate(rich = ifelse(GDP_agri_destination_quartile == 1, 1, 0))

z <- migration %>%
  filter(GDP_agri_destination_tertile != 3) %>%
  mutate(rich = ifelse(GDP_agri_destination_tertile == 1, 1, 0))
